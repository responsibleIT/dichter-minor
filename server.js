  ...
  switch (poemType) {
    case "limerick":
      messages.push({
        role: "system",
        content:
          "Schrijf een limerick van vijf regels met een metrum van twee regels met drie amfibrachen, " +
    ...
    case "elevenie":
      messages.push({
        role: "system",
        content:
          "Schrijf een elfje van elf woorden op vijf dichtregels. " +
    ...
    case "ollekebolleke":
      messages.push({
        role: "system",
        content:
          "Schrijf een Ollekebolleke met twee strofen van elk vier versregels met schema abcd efgd gebruik metrum dactylus met varianten.",
      });
      break;
    case "snelsonnet":
      messages.push({
        role: "system",
        content:
          "Schrijf een snelsonnet met een kwatrijn van 4 regels, gevolgd door een distichon van 2 regels. " +
    ...
    case "haiku":
      messages.push({
        role: "system",
        content:
          "Schrijf een Haiku van drie regels waarvan de eerste regel 5, de tweede regel 7 en de derde regel weer 5 lettergrepen telt.",
      });
      break;
    ...
    case "toneel opening tragedie":
      messages.push({
        role: "system",
        content:
          "Schrijf een toneel dialoog van een tragedie voor de eerste akte.",
      });
      break;
    case "toneel opening komedie":
      messages.push({
        role: "system",
        content:
          "Schrijf een toneel dialoog van een komedie voor de eerste akte.",
      });
      break;
  }
...
function api_generate(poemType, themes) {
  ...
  msgs.push({
    role: "system",
    content:
      "Output format in a valid JSON like {paragraph: '...'} Also use proper punctuation, no weird characters like newlines.",
  });
...
const start = async () => {
  ...
  await fastify.register(auth).register(bearerAuthPlugin, {
    addHook: false,
    keys,
  ...
  fastify.route({
    ...
    handler: async (request, reply) => {
      let poemType =
        poemTypes[request.query["type"]] ||
        Object.keys(poemTypes).find(
          (key) => poemTypes[key] === request.query["type"]
        );
      ...
      try {
        ...
        if (!oldWord || !newWord) {
          reply.status(400).send({
            message: "Missing old and / or new word query parameters",
          });
        }
  ...
  fastify.get("/", async (request, reply) => {
    ...
    reply
      ...
      .view("views/index.eta", { title: "Stadsdichter" });
  });  
  ...
  fastify.get("/filter", async (request, reply) => {
    ...
    reply
      ...
      .view("views/filter.eta", { title: "Stadsdichter" });
  });

  fastify.get("/docs", async (request, reply) => {
    reply.view("views/docs.eta", {
      title: "Docs | Stadsdichter",
      bk: process.env.BK,
  ...
  fastify.post("/", async (request, reply) => {
    ...
    if (!cookie || !fastify.unsignCookie(cookie).valid) {
      ...
      let poemType =
        poemTypes[request.body["type"]] ||
        Object.keys(poemTypes).find(
          (key) => poemTypes[key] === request.body["type"]
        );
